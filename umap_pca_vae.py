...
# Function to load precomputed VAE latent space representations
def load_vae_latent_space(file_path):
...
if 'features' in locals() and 'labels' in locals():
    ...
    if analysis_type == "UMAP":
        ...
        vae_results = load_vae_latent_space('vae_latent_space.pkl')
    ...
    if analysis_type == "VAE":
        st.write("### Generated Digits in Latent Space")
        fig, axes = plt.subplots(1, 5, figsize=(10, 3))
        for i in range(5):
            axes[i].imshow(images[i], cmap='gray')
