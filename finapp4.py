...
# pip install sentence-transformers streamlit plotly matplotlib umap-learn tensorflow
...
def load_animal_descriptions(vectorization_method):
    df = pd.read_csv('animal_descriptions.csv')
    if vectorization_method == 'TF-IDF':
        vectorizer = TfidfVectorizer()
        tfidf_matrix = vectorizer.fit_transform(df["Description"])
        features = pd.DataFrame(tfidf_matrix.toarray(), index=df["Animal"], columns=vectorizer.get_feature_names_out())
    elif vectorization_method == 'Sentence2Vec':
        model = SentenceTransformer('paraphrase-MiniLM-L6-v2')
        sentence_embeddings = model.encode(df["Description"].tolist())
        features = pd.DataFrame(sentence_embeddings, index=df["Animal"])
    labels = df["Animal"]
    return features, labels, df

def load_naics_codes(vectorization_method):
    df = pd.read_csv('naics_codes.csv')
    if vectorization_method == 'TF-IDF':
        vectorizer = TfidfVectorizer()
        tfidf_matrix = vectorizer.fit_transform(df["Description"])
        features = pd.DataFrame(tfidf_matrix.toarray(), index=df["NAICS Code"], columns=vectorizer.get_feature_names_out())
    elif vectorization_method == 'Sentence2Vec':
        model = SentenceTransformer('paraphrase-MiniLM-L6-v2')
        sentence_embeddings = model.encode(df["Description"].tolist())
        features = pd.DataFrame(sentence_embeddings, index=df["NAICS Code"])
    labels = df["NAICS Code"]
    return features, labels, df

def load_financial_statements(vectorization_method):
    df = pd.read_csv('financial_statements.csv')
    if vectorization_method == 'TF-IDF':
        vectorizer = TfidfVectorizer()
        tfidf_matrix = vectorizer.fit_transform(df["Description"])
        features = pd.DataFrame(tfidf_matrix.toarray(), index=df["Company"], columns=vectorizer.get_feature_names_out())
    elif vectorization_method == 'Sentence2Vec':
        model = SentenceTransformer('paraphrase-MiniLM-L6-v2')
        sentence_embeddings = model.encode(df["Description"].tolist())
        features = pd.DataFrame(sentence_embeddings, index=df["Company"])
    labels = df["Company"]
    return features, labels, df
...
def intro():
    ...
    if st.button("Go to Explanation"):
...
def app():
    ...
    if dataset_choice in ["Animal Descriptions (Text Data)", "NAICS Codes (Text Data)", "Financial Statements (Text Data)"]:
        vectorization_method = st.sidebar.selectbox("Select vectorization method", ["TF-IDF", "Sentence2Vec"])
    ...
    if not submit:
        ...
        if dataset_choice == "Upload your own CSV file":
            ...
            if uploaded_file is not None:
                ...
                st.write("### First 10 Lines of the Uploaded Data (first 10 rows are shown)")
                st.write(df.head(10))
            ...
            if dataset_choice == "Digits MNIST (Image Data)":
                ...
                st.write("This dataset contains 100 sentences describing various animals. Each description can be vectorized using TF-IDF or Sentence2Vec.")
                st.write("**Data Points/Rows:** 100")
                st.write("**Features/Columns:** Varies by vectorization method")
                features, labels, df = load_animal_descriptions(vectorization_method)
                st.write("### Animal Descriptions Dataset")
                st.write(df)
                st.write(f"### {vectorization_method} Vectors for Animal Descriptions")
                st.write(features)

            elif dataset_choice == "NAICS Codes (Text Data)":
                st.write("### NAICS Codes Dataset")
                st.write("This dataset contains descriptions of various North American Industry Classification System (NAICS) codes. Each description can be vectorized using TF-IDF or Sentence2Vec.")
                st.write("**Data Points/Rows:** 81")
                st.write("**Features/Columns:** Varies by vectorization method")
                features, labels, df = load_naics_codes(vectorization_method)
                st.write("### NAICS Codes Dataset")
                st.write(df)
                st.write(f"### {vectorization_method} Vectors for NAICS Codes")
                st.write(features)

            elif dataset_choice == "Financial Statements (Text Data)":
                st.write("### Financial Statements Dataset")
                st.write("This dataset contains descriptions of financial statements from various companies. Each description can be vectorized using TF-IDF or Sentence2Vec.")
                st.write("**Data Points/Rows:** 65")
                st.write("**Features/Columns:** Varies by vectorization method")
                st.write("**Note:** Data was taken from ChatGPT Q1 2024")
                features, labels, df = load_financial_statements(vectorization_method)
                st.write("### Financial Statements Dataset")
                st.write(df)
